...
| `AGENT_AUTO_REVIEW_STALL_THRESHOLD` | number  | `3`                            | Consecutive appearances of the same auto-review blocker fingerprint before the loop is treated as stalled and handed off for manual review. Minimum `1`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      |
...
| `AIF_MEMORY_ENABLED`                | boolean | `true`                         | Master switch for server-side product memory. When `true`, approving a done task to verified creates a pending SQLite-backed memory candidate, the Memory UI can review/approve/reject/expire it, and approved project/global memory is retrieved for planner, implementer, reviewer, security review, and chat prompts as bounded source-backed reference-only context with usage auditing. Approval requires clean redaction status and at least one valid source-backed claim. When `false`, candidate creation, retrieval, prompt injection, and memory usage writes are skipped                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
| `AIF_WARMUP_ENABLED`                | boolean | `false`                        | Master switch for project runtime warmup. When `false`, `/projects/:id/warmup` reports `enabled=false`, create requests return `403`, and the web UI hides Warmup entry points. When `true`, supported planner, implementer, reviewer, security, audit, and synthesis runtimes can create time-limited seed sessions that later matching stage runs may fork. Warmup lifecycle logs use the `projects-route`, `api-runtime`, and runtime adapter log components and include project/runtime ids, TTL, expiry, and status without prompt text or secrets                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      |
...
time-limited seed sessions for a project's effective planner, implementer,
reviewer, security, audit, and synthesis stages and stores them in
`runtime_warmup_sessions`. Rows are scoped by canonical `stage` plus
runtime/profile/model, so stages that share a compatibility profile still keep
separate seed sessions. Later stage runs can fork a seed only when their
effective stage and runtime/profile/model still match the warmup row.
...
Each runtime run also carries a provider-neutral permission policy selected from task intent:

| Intent                      | Default mode            | Write boundary                   |
| --------------------------- | ----------------------- | -------------------------------- |
| `feature`, `fix`, `general` | `workspace_write`       | approved task scope              |
| `tests`                     | `workspace_write`       | tests, specs, and fixtures       |
| `docs`                      | `workspace_write`       | documentation files only         |
| `spike`                     | `read_only`             | research-only by default         |
| `audit`                     | `audit_diagnostic_only` | diagnostic report artifacts only |

The canonical modes are `danger_full_access`, `workspace_write`, `read_only`, `review_only`, and `audit_diagnostic_only`. Audit tasks must not modify source, config, or test files; docs tasks must not modify source files. Dangerous shell commands such as recursive deletion, credential reads, permission changes, network transfer commands, and destructive git operations require human approval and fail closed when no approval bridge is available.
...
Bypass mode is visible in task activity as `[permission-policy:bypass]` when a task-scoped run enables it. If a task intent disallows bypass, such as `audit`, the runtime clears native provider bypass and records `[permission-policy:bypass-blocked]` instead.
