...
describe("runImplementer rework behavior", () => {
  ...
  it("normalizes empty tracked audit scope files deterministically before runtime", async () => {
    ...
    expect(validation.sourceClassification).not.toBe("validated_no_findings");
  });
  ...
  it("routes governance-only audit report rework back to runtime before repair marker", async () => {
    const db = testDb.current;
    execFileSync("git", ["init", "-b", "main"], { cwd: projectRoot, stdio: "ignore" });
    execFileSync("git", ["config", "user.email", "test@example.com"], {
      cwd: projectRoot,
  ...
  it("routes structurally invalid audit validator reports back to runtime before repair marker", async () => {
    const db = testDb.current;
    execFileSync("git", ["init", "-b", "main"], { cwd: projectRoot, stdio: "ignore" });
    execFileSync("git", ["config", "user.email", "test@example.com"], {
      cwd: projectRoot,
    ...
    expect(repaired).toContain("would show");
    expect(repaired).not.toContain("nf-deterministic-repair");
  ...
  it("throws controlled budget exhaustion when timeout recovery has no substantive ledger evidence", async () => {
    ...
    createRoadmapBatchContract({
      projectId: "project-1",
      roadmapAlias: "audit-timeout-ledger-empty",
      taskIntent: "audit",
      executionPolicy: "serialized_shared_checkout",
      createdTaskIds: ["task-audit-timeout-ledger-empty"],
      artifacts: [
        {
          taskId: "task-audit-timeout-ledger-empty",
          role: "report",
          artifactPath: "audit/architecture.md",
          projectRoot,
        },
    ...
    await expect(
      runImplementer("task-audit-timeout-ledger-empty", projectRoot),
    ).rejects.toMatchObject({
      category: "context_length",
      providerMeta: expect.objectContaining({
        status: "repository_inspection_budget_exhausted",
        reason: "zero_substantive_ledger_evidence",
      }),
    });
    expect(queryMock).toHaveBeenCalledTimes(1);
    const updatedTask = db
  ...
  it("skips ledger-writer recovery when deterministic timeout repair succeeds", async () => {
    ...
    db.insert(tasks)
      .values({
        id: "task-audit-budget-ledger-writer-fail",
        projectId: "project-1",
        title: "Audit architecture",
        description: [
          "Scope: README.md",
    ...
    createRoadmapBatchContract({
      projectId: "project-1",
      roadmapAlias: "audit-budget-ledger-writer-fail",
      taskIntent: "audit",
      executionPolicy: "serialized_shared_checkout",
      createdTaskIds: ["task-audit-budget-ledger-writer-fail"],
      artifacts: [
        {
          taskId: "task-audit-budget-ledger-writer-fail",
          role: "report",
          artifactPath: "audit/architecture.md",
          projectRoot,
        },
    ...
    const artifact = findRoadmapBatchArtifactByTaskId("task-audit-budget-ledger-writer-fail");
    if (!artifact) throw new Error("missing report artifact");
    appendAuditEvidenceEvent({
      id: "ev_budget_writer_00000000-0000-4000-8000-000000000001",
      taskId: "task-audit-budget-ledger-writer-fail",
      auditPlanId: `batch:${artifact.batchId}:task:task-audit-budget-ledger-writer-fail`,
      sourceSnapshotId: `git:${head}:tree`,
      toolName: "read_file",
      evidenceKind: "file_read",
      evidenceGrade: "substantive",
  ...
  it("does not use hidden tooling files as broad deterministic repair evidence", async () => {
    ...
    db.insert(tasks)
      .values({
        ...
        useSubagents: true,
        blockedReason:
          "invalid_artifact_content: audit_evidence_repair_required (missing_report_manifest): missing source report manifest.",
  ...
  it("fails closed when a required implementation manifest is omitted even if deterministic evidence exists", async () => {
    ...
    queryMock
      .mockReturnValueOnce({
        async *[Symbol.asyncIterator]() {
          appendAuditEvidenceEvent({
            id: "ev-feature-fallback-build",
            taskId: "task-feature-fallback-manifest",
            auditPlanId: "task:task-feature-fallback-manifest",
          ...
          const current = db
            .select()
            .from(tasks)
            .where(eq(tasks.id, "task-feature-fallback-manifest"))
            .get();
          db.update(tasks)
            .set({
              agentActivityLog: [
    ...
    db.insert(tasks)
      .values({
        id: "task-feature-fallback-manifest",
        projectId: "project-1",
        title: "Feature fallback manifest",
        description: "Add a small feature.",
        taskIntent: "feature",
        status: "implementing",
        plan,
    ...
    await runImplementer("task-feature-fallback-manifest", projectRoot);
  ...
  it("fails closed when extracted normalized implementation manifest changedFiles drift from actual git changes", async () => {
    ...
    const plan = [
      ...
      JSON.stringify({
        version: 1,
        taskId: "task-feature-repair-manifest",
        intent: "feature",
        scope: ["src/feature.ts"],
        allowedChanges: ["source"],
        forbiddenChanges: ["audit-report"],
    ...
    queryMock
      .mockReturnValueOnce({
        async *[Symbol.asyncIterator]() {
          appendAuditEvidenceEvent({
            id: "ev-feature-repair-test",
            taskId: "task-feature-repair-manifest",
            auditPlanId: "task:task-feature-repair-manifest",
            sourceSnapshotId: "git:test",
            toolName: "run_shell",
            evidenceKind: "shell_command",
            evidenceGrade: "substantive",
          ...
          const current = db
            .select()
            .from(tasks)
            .where(eq(tasks.id, "task-feature-repair-manifest"))
            .get();
          db.update(tasks)
            .set({
              agentActivityLog: [
          ...
          yield {
            ...
            subtype: "success",
            result: [
              "Implementation done with a stale manifest.",
              "",
              "```aif-implementation-manifest",
              JSON.stringify({
                version: 1,
                ...
                changedFiles: [],
                diffSummary: { summary: "No changes required.", filesChanged: 0 },
                verificationEvidence: [
                  {
                    id: "verify-1",
                    command: "npm test",
                ...
                evidenceRefs: ["verify-1"],
                planChecklist: { total: 0, completed: 0, pending: 0, synced: true },
                reviewClosure: { status: "pending", evidenceRefs: [] },
                commitEvidence: { status: "not_required", evidenceRefs: [] },
                knownLimitations: [],
              }),
    ...
    db.insert(tasks)
      .values({
        id: "task-feature-repair-manifest",
        projectId: "project-1",
        title: "Feature repair manifest",
        description: "Add a small feature.",
        taskIntent: "feature",
        status: "implementing",
        plan,
    ...
    await runImplementer("task-feature-repair-manifest", projectRoot);
  ...
  it("does not use deterministic fallback to override invalid rework implementation manifests", async () => {
    ...
    execFileSync("git", ["checkout", "-b", "feature/rework-manifest"], {
      cwd: projectRoot,
      stdio: "ignore",
    });
    writeFileSync(join(projectRoot, "src", "feature.ts"), "export const feature = true;\n");
    ...
    queryMock.mockReturnValueOnce({
      async *[Symbol.asyncIterator]() {
        appendAuditEvidenceEvent({
          id: "ev-feature-rework-test",
          taskId: "task-feature-rework-manifest",
          auditPlanId: "task:task-feature-rework-manifest",
          sourceSnapshotId: "git:test",
          toolName: "run_shell",
          evidenceKind: "shell_command",
          evidenceGrade: "substantive",
          ...
          outputPreviewTruncated: false,
          parsedSummary: {
            outputBytes: "fresh 18 tests passed".length,
            outputLineCount: 1,
            previewChars: "fresh 18 tests passed".length,
            exitCode: 0,
          },
          redactionStatus: "clean",
          createdAt: verifiedAt,
        ...
        const current = db
          .select()
          .from(tasks)
          .where(eq(tasks.id, "task-feature-rework-manifest"))
          .get();
        db.update(tasks)
          .set({
            agentActivityLog: [
        ...
        yield {
          ...
          subtype: "success",
          result: [
            "Implementation done with a stale but structurally valid manifest.",
            "",
            "```aif-implementation-manifest",
            JSON.stringify({
              version: 1,
              ...
              verificationEvidence: [
                {
                  ...
                  status: "passed",
                  outputSha256: "1".repeat(64),
                  outputPreview: "old 9 tests passed",
                  outputPreviewTruncated: false,
                },
              ],
    ...
    db.insert(tasks)
      .values({
        id: "task-feature-rework-manifest",
        projectId: "project-1",
        title: "Feature rework manifest",
        description: "Add a small feature.",
        taskIntent: "feature",
        status: "implementing",
        plan,
    ...
    await runImplementer("task-feature-rework-manifest", projectRoot);
