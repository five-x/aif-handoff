...
describe("coordinator", () => {
  ...
  it("validates a written audit artifact before blocking post-write transport failures", async () => {
    const db = testDb.current;
    const rootPath = initGitFixture("coordinator-audit-post-write-transport-");
    mkdirSync(join(rootPath, "audit"), { recursive: true });
    writeFileSync(
    ...
    db.insert(tasks)
      .values({
        id: "task-audit-post-write-transport",
        projectId: "test-project",
        title: "Audit architecture",
        description: "Scope: README.md\nReport artifact: audit/architecture.md",
        taskIntent: "audit",
    ...
    const batch = createRoadmapBatchContract({
      projectId: "test-project",
      roadmapAlias: "audit-post-write-transport",
      taskIntent: "audit",
      executionPolicy: "serialized_shared_checkout",
      createdTaskIds: ["task-audit-post-write-transport"],
      synthesisTaskId: null,
      artifacts: [
        {
          taskId: "task-audit-post-write-transport",
          role: "report",
          artifactPath: "audit/architecture.md",
          projectRoot: rootPath,
        },
    ...
    const retrying = db
      .select()
      .from(tasks)
      .where(eq(tasks.id, "task-audit-post-write-transport"))
      .get();
    ...
    expect(retrying!.blockedReason).not.toBe(
      "Runtime request failed. Task will retry automatically.",
    );
    expect(retrying!.agentActivityLog).toContain(
    ...
    const artifact = listRoadmapBatchArtifacts(batch.batchId).find(
      (entry) => entry.taskId === "task-audit-post-write-transport",
    );
    ...
    const validatorIssueCodes =
      details.evidence?.auditReportValidation?.issues?.map(
        (entry: { code: string }) => entry.code,
      ) ?? [];
    expect(validatorIssueCodes).toEqual(
      expect.arrayContaining(["synthetic_git_output", "non_actionable_audit_observation"]),
    );
  });
