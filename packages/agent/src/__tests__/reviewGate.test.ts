...
describe("evaluateReviewCommentsForAutoMode", () => {
  ...
  it("keeps Russian repository evidence requests in implementation rework", async () => {
    const result = await evaluateReviewCommentsForAutoMode({
      ...baseInput,
      reviewComments: [
        "## Auto Review Metadata",
        ...
        "",
        "## Blocking Findings",
        "- [repo-ru-1] security_audit | operator_input_required: предоставьте акт сканирования секретов и вывод команды npm test для package.json",
        "",
        "## Advisories",
        "- security_audit | none",
        "",
    ...
    expect(result.blockingFindings[0]?.id).toBe("repo-ru-1");
  });
