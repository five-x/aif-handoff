...
function formatAuditReportManifestContractForPrompt(input: {
  ...
  return [
    ...
    `  - In qwen-local-agent, call tool \`compute_audit_report_hash\` with path \`${input.artifactPath}\` and use the returned 64-hex value.`,
    "  - In shell-capable runtimes, use this one-line command:",
    `  - ${contentHashCommand}`,
    "- Never use PLACEHOLDER, <computed_sha256>, all-zero hashes, shortened hashes, or any non-64-hex value. If the hash cannot be computed, do not claim the report is complete.",
    "- Do not create temporary helper files, scratch scripts, or root-level `tmp_*` files to compute the hash. Use the runtime hash tool or the one-line command above and write only the expected audit report artifact.",
