...
function formatImplementationManifestPrompt(
  ...
  const verificationInstruction =
    planManifest.verificationCommands.length > 0
      ? `\n- Required verification command(s) from the approved plan: ${planManifest.verificationCommands.map((command) => `\`${command}\``).join(", ")}. In qwen-local-agent, execute package-manager verification with \`run_shell\` (for example command \`npm.cmd\`, args \`[\"run\", \"build\"]\`); the tool maps \`npm.cmd\` to \`npm\` on Linux. If dependencies changed or \`node_modules\` is missing, run dependency hydration first with \`npm.cmd install\`; package specs are not allowed and lifecycle scripts/audit/fund are disabled by the tool. Never run \`npm run dev\`, \`npm start\`, watch, preview, serve, or long-running development server commands for verification. Do not mark \`read_file\`, \`list_files\`, or \`git_status\` as passed verification for build/test criteria.`
      : "\n- Execute the concrete build/test/lint verification command with `run_shell` before marking verification evidence as passed; never run `npm run dev`, `npm start`, watch, preview, serve, or long-running development server commands for verification; do not substitute repository inspection tools for build/test verification.";
...
function buildDeterministicAuditReportRepairContent(input: {
  ...
  for (const entry of evidenceByRoot) {
    const exactLineRefs = entry.lineRefs.filter((ref) => /:\d+$/.test(ref));
    ...
    if (exactLineRefs.length === 0) {
      decisionReasons.push(`Scope root ${entry.root} has no exact source line evidence.`);
      continue;
    }
...
- Commit only ${expectedAuditReportArtifactPath}, verify with git log -1 --name-only --oneline -- ${expectedAuditReportArtifactPath}, then return a concise result.`;
    ...
    auditLedgerWriterRecoveryInvoked = true;
