...
export function assertRemoteOnlyApiTarget({ baseUrl, skipDevServer }) {
  if (skipDevServer !== "0" && isLocalApiUrl(baseUrl)) {
    throw new Error(
      "Local k6 API validation requires explicit local opt-in: set AIF_SKIP_DEV_SERVER=0.",
    );
  }
}
