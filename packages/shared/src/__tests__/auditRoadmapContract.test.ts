...
function completeAuditDescription(options: { synthesis?: boolean } = {}) {
  ...
  const reportPath = synthesis ? "audit/summary.md" : "audit/config-audit.md";
  return [
    synthesis
...
describe("auditRoadmapContract", () => {
  ...
  it("rejects generated audit cards that omit explicit child contract metadata", () => {
    const invalid = validateGeneratedAuditCard({
      title: "Audit: security configuration",
      description: completeAuditDescription()
        .replace(/^Task intent:.*\n/m, "")
        .replace(/^Expected report artifact:.*\n/m, "")
        .replace(/^Allowed write paths:.*\n/m, "")
        .replace(/^Dependency order:.*\n/m, "")
        .replace(/^Trusted artifact lifecycle:.*\n/m, ""),
    });
