...
import {
  ...
  type AuditEvidenceDepthReasonCode,
  type AuditEvidenceDepthStatus,
...
export interface AuditCorpusReportCase {
  ...
  expectedEvidenceDepthStatus?: AuditEvidenceDepthStatus;
  expectedEvidenceDepthReasonCodes?: AuditEvidenceDepthReasonCode[];
...
export function initAuditContractRepo(): string {
  ...
  writeFileSync(
    join(root, "src", "importOnly.ts"),
    ['import { timeoutMs } from "./config";', ""].join("\n"),
    "utf8",
  );
  writeFileSync(
    join(root, "src", "types.ts"),
    ["export interface RuntimePolicy {", "  mode: string;", "}", ""].join("\n"),
    "utf8",
  );
  writeFileSync(
    join(root, "src", "auth.ts"),
    ["export const timeoutMs = 1000;", ""].join("\n"),
    "utf8",
  );
  writeFileSync(
  ...
  writeFileSync(
    ...
    [
      ...
      'expect(runtimeBoundary(" value ")).toBe("value");',
      "",
    ].join("\n"),
    "utf8",
  );
...
export const invalidAuditReportCases: AuditCorpusReportCase[] = [
  {
    ...
    expectedEvidenceDepthStatus: "shallow",
    expectedEvidenceDepthReasonCodes: [
      "inventory_only_evidence",
      "irrelevant_grep_match",
      "shallow_evidence",
    ],
    expectedTrustedNoFindingsSupported: false,
  },
  {
  ...
  },
  {
    ...
    title: "import-only evidence cannot prove no-findings",
    artifactPath: "audit/import-only.md",
    body: [
      "# Import Boundary Audit",
      "",
      "No validated findings.",
      "Risk hypotheses: risk-import for `src/importOnly.ts` import boundary drift was covered and is absent.",
      "",
      "Checked files:",
      "- `src/importOnly.ts:1`",
      "",
      "Checked commands:",
      '- Command `rg -n "import" src/importOnly.ts` output:',
      "```",
      'src/importOnly.ts:1:import { timeoutMs } from "./config";',
      "```",
      "",
    ].join("\n"),
    scopeRoots: ["src/importOnly.ts"],
    expectedClassification: "inventory_only_invalid",
    expectedIssueCodes: ["missing_substantive_evidence"],
    expectedEvidenceDepthStatus: "shallow",
    expectedEvidenceDepthReasonCodes: ["insufficient_scope_depth", "shallow_evidence"],
    expectedTrustedNoFindingsSupported: false,
  },
  {
    id: "declaration-only-snippet",
    title: "declaration-only snippets cannot prove behavior no-findings",
    artifactPath: "audit/declaration-only.md",
    body: [
      "# Runtime Policy Audit",
      "",
      "No validated findings.",
      "Risk hypotheses: risk-execution for `src/types.ts` execution enforcement was covered and is absent.",
      "",
      "Checked files:",
      "- `src/types.ts:1`",
      "",
      "Checked commands:",
      '- Command `rg -n "RuntimePolicy" src/types.ts` output:',
      "```",
      "src/types.ts:1:export interface RuntimePolicy {",
      "```",
      "",
    ].join("\n"),
    scopeRoots: ["src/types.ts"],
    expectedClassification: "source_inconclusive",
    expectedIssueCodes: ["irrelevant_grep_match", "shallow_evidence"],
    expectedEvidenceDepthStatus: "shallow",
    expectedEvidenceDepthReasonCodes: ["irrelevant_grep_match", "shallow_evidence"],
    expectedTrustedNoFindingsSupported: false,
  },
  {
    id: "quoted-dot-grep-output",
    title: "quoted dot-grep output is inventory evidence",
    artifactPath: "audit/quoted-dot-grep.md",
    body: [
      "# Config Audit",
      "",
      "No validated findings.",
      ...
      "",
      "Checked commands:",
      '- Command `rg -n "." src/config.ts` output:',
      "```",
    ...
    scopeRoots: ["src/config.ts"],
    expectedClassification: "inventory_only_invalid",
    expectedIssueCodes: ["inventory_only_evidence", "irrelevant_grep_match"],
    expectedEvidenceDepthStatus: "shallow",
    expectedEvidenceDepthReasonCodes: ["inventory_only_evidence", "irrelevant_grep_match"],
    expectedTrustedNoFindingsSupported: false,
  },
  {
    id: "generic-dot-grep-output",
    title: "generic dot-grep output is inventory evidence",
    artifactPath: "audit/generic-dot-grep.md",
    body: [
      "# Config Audit",
      "",
      "No validated findings.",
  ...
  {
    id: "loose-grep-match",
    title: "loose grep matches cannot prove a specific risk",
    artifactPath: "audit/loose-grep.md",
    body: [
      "# Config Audit",
      "",
      "No validated findings.",
      "Risk hypotheses: risk-retry for `src/config.ts` retry drift was covered and is absent.",
      "",
      "Checked files:",
      "- `src/config.ts:1`",
      "- `src/config.ts:3`",
      "",
      "Checked commands:",
      '- Command `rg -n "const" src/config.ts` output:',
      "```",
      "src/config.ts:1:export const timeoutMs = 1000;",
      "src/config.ts:3:export const retryLimit = 3;",
      "```",
      "",
    ].join("\n"),
    scopeRoots: ["src/config.ts"],
  ...
  {
    id: "path-only-risk-term-match",
    title: "risk terms matched only by the path are ignored",
    artifactPath: "audit/path-only-risk.md",
    body: [
      "# Auth Audit",
      "",
      "No validated findings.",
      ...
      "",
      "Checked commands:",
      '- Command `rg -n "timeoutMs" src/auth.ts` output:',
      "```",
      "src/auth.ts:1:export const timeoutMs = 1000;",
      "```",
      "",
    ].join("\n"),
  ...
  {
    id: "reused-snippet-for-unrelated-risks",
    title: "reused snippets cannot prove unrelated no-findings risks",
    artifactPath: "audit/reused-snippet.md",
    body: [
      "# Config Audit",
      "",
      "No validated findings.",
  ...
  {
    id: "self-reported-command-output",
    title: "self-reported command output is not observed evidence",
    artifactPath: "audit/self-reported-output.md",
    body: [
      "# Config Audit",
      "",
      "No validated findings.",
      "Scoped no-findings claim: `src/config.ts` timeout risk is absent.",
      "",
      "Checked files:",
      "- `src/config.ts:1`",
      "",
      "Checked commands:",
      '- Command `rg -n "." src/config.ts` output would show the timeout.',
      "",
    ].join("\n"),
    scopeRoots: ["src/config.ts"],
    expectedClassification: "inventory_only_invalid",
    expectedIssueCodes: ["unverified_inspection_claim", "inventory_only_evidence"],
    expectedEvidenceDepthStatus: "shallow",
    expectedEvidenceDepthReasonCodes: [
      "inventory_only_evidence",
      "irrelevant_grep_match",
  ...
  {
    id: "mixed-no-risk-scoped-claim",
    title: "mixed no-risk scoped claims require risk-specific evidence",
    artifactPath: "audit/mixed-no-risk.md",
    body: [
      "# Runtime Audit",
      "",
      "No validated findings.",
      "Risk hypotheses: risk-timeout for `src/config.ts` timeout configuration was covered and is absent.",
      "Scoped no-findings claim: `src/runtime.ts` queue deadlock risk is absent.",
      "",
      "Checked files:",
      "- `src/config.ts:1`",
      "- `src/runtime.ts:1`",
      ...
      "Checked commands:",
      '- Command `rg -n "timeoutMs" src/config.ts` output: `src/config.ts:1:export const timeoutMs = 1000;`',
      '- Command `rg -n "runtimeBoundary" src/runtime.ts` output: `src/runtime.ts:1:export function runtimeBoundary(input: string) {`',
      "",
    ].join("\n"),
    scopeRoots: ["src/config.ts", "src/runtime.ts"],
    expectedClassification: "source_inconclusive",
    expectedIssueCodes: ["irrelevant_grep_match", "shallow_evidence"],
    expectedEvidenceDepthStatus: "shallow",
    expectedEvidenceDepthReasonCodes: ["irrelevant_grep_match", "shallow_evidence"],
  ...
  },
  {
    ...
    body: [
      ...
      "",
      "No validated findings.",
      "Scoped no-findings claim: `src/persistence.ts` persistenceOwner ownership risk is absent.",
      "",
      "Checked files:",
      "- `src/persistence.ts:1`",
      "",
...
export const validNoFindingsAuditReportCases: AuditCorpusReportCase[] = [
  {
    ...
    expectedEvidenceDepthStatus: "substantive",
    expectedEvidenceDepthReasonCodes: [],
    expectedTrustedNoFindingsSupported: true,
    evidence: {
    ...
    body: [
      ...
      "",
      "No validated findings.",
      "Risk hypotheses: risk-security-config for authMode and timeoutMs configuration coverage was covered and is absent.",
  ...
  },
  {
    ...
    body: [
      ...
      "",
      "No validated findings.",
      "Risk hypotheses: risk-runtime-boundary for runtimeBoundary and trim normalization coverage was covered and is absent.",
  ...
  },
  {
    ...
    body: [
      ...
      "",
      "No validated findings.",
      "Risk hypotheses: risk-persistence-ownership for persistenceOwner and writeAuditState ownership coverage was covered and is absent.",
  ...
  },
  {
    ...
    body: [
      ...
      "",
      "No validated findings.",
      "Risk hypotheses: risk-ops-config for Rollback and config validation coverage was covered and is absent.",
  ...
  },
  {
    ...
    body: [
      ...
      "",
      "No validated findings.",
      "Risk hypotheses: risk-architecture-boundary for architectureBoundary and allowedLayer coverage was covered and is absent.",
  ...
  {
    id: "empty-file-proof",
    title: "empty-file proof no-findings",
    artifactPath: "audit/empty-file.md",
    ...
    expectedTrustedNoFindingsSupported: true,
    evidence: {
      scopeIds: ["tests/__init__.py"],
      riskHypothesisIds: ["risk-empty-marker"],
      command: "git hash-object -- tests/__init__.py",
      outputPreview: "e69de29bb2d1d6434b8b29ae775ad8c2e48c5391",
    },
    body: [
      "# Empty Marker Audit",
      "",
      "No validated findings.",
      "Risk hypotheses: risk-empty-marker for `tests/__init__.py` empty package marker coverage was covered and is absent.",
      "Absence reasoning: `tests/__init__.py` is intentionally empty and the hash-object proof matches the empty blob.",
      "",
      "Checked files:",
      "- `tests/__init__.py`",
      "",
  ...
  {
    id: "targeted-runtime-test-output",
    title: "targeted runtime/test output no-findings",
    artifactPath: "audit/runtime-test-output.md",
    scopeRoots: ["tests/runtime.test.ts"],
    expectedClassification: "validated_no_findings",
    expectedEvidenceDepthStatus: "substantive",
    expectedEvidenceDepthReasonCodes: [],
    expectedTrustedNoFindingsSupported: true,
    evidence: {
      scopeIds: ["tests/runtime.test.ts"],
      riskHypothesisIds: ["risk-runtime-test"],
      command: 'rg -n "runtimeBoundary|toBe" tests/runtime.test.ts',
      outputPreview:
        'tests/runtime.test.ts:1:import { runtimeBoundary } from "../src/runtime";\ntests/runtime.test.ts:2:expect(runtimeBoundary(" value ")).toBe("value");',
    },
    body: [
      "# Runtime Test Audit",
      "",
      "No validated findings.",
      "Risk hypotheses: risk-runtime-test for `tests/runtime.test.ts` runtimeBoundary trim expectation coverage was covered and is absent.",
      "Absence reasoning: `tests/runtime.test.ts:2` asserts the trimmed runtimeBoundary value, so the scoped runtime test risk is absent.",
      "",
      "Checked files:",
      "- `tests/runtime.test.ts:2`",
      "",
      "Checked commands:",
      '- Command `rg -n "runtimeBoundary|toBe" tests/runtime.test.ts` output:',
      "```",
      'tests/runtime.test.ts:1:import { runtimeBoundary } from "../src/runtime";',
      'tests/runtime.test.ts:2:expect(runtimeBoundary(" value ")).toBe("value");',
      "```",
