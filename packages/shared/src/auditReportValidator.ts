...
function textMakesAbsenceClaimFromSkippedSearch(
  ...
  const absencePattern =
    /\b(?:zero\s+matches|0\s+matches|no\s+matches|returned\s+zero\s+matches|not\s+imported|not\s+wired|no\s+(?:imports?|references?|callers?|wired\s+caller|path\s+to|declared\s+caller|(?:cli\s+)?commands?\s+exists?)|no (?:cli )?command exists|orphaned|dead\s+code|unused)\b/i;
